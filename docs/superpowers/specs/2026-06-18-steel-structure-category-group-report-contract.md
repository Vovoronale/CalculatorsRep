...
For the default C245 case, step 5 retains the agreed calculated resistance, not a rounded table substitution:
...
static load and has_tensile_stress = No and sigma_c <= sigma_limit => delta_Scompression = -4
otherwise => delta_Scompression = 0
```

If tensile stress exists, the static-compression reduction is not applicable even when `sigma_c <= sigma_limit`.
...
```text
За таблицею А.1 конструкцію «<structure_label>» віднесено до категорії <А/Б/В> за призначенням і категорії <I/II/III> за напруженим станом.
```
...
Категорію визначено для конструкції, прямо наведеної в таблиці А.1. Віднесення інших конструкцій за аналогією відповідно до примітки 2 потребує окремого інженерного обґрунтування.
```
...
```text
S_1 = <0/4> <бал/бали/балів> — клас відповідальності <СС1/СС2/СС3>.
S_2 = <11/4/1> <бал/бали/балів> — категорія за призначенням <А/Б/В>.
S_3,base = <8/5/1> <бал/бали/балів> — категорія за напруженим станом <I/II/III>.
S_4 = <7/2> <бал/бали/балів> — розтягувальні напруження <є/немає>.
S_5 = <6/2> <бал/бали/балів> — несприятливий вплив зварних з’єднань <є/немає>.
```

Note:

```text
Несприятливий вплив зварних з’єднань враховується, якщо вони розташовані у місцях дії значних розрахункових розтягувальних напружень (σ > 0,3R_y; σ > 0,3R_wz), або в місцях, де міцність зварного з’єднання визначає придатність до експлуатації конструкції в цілому (примітка до таблиці А.2).
```
...
```text
S_tot,base = S_1 + S_2 + S_3,base + S_4 + S_5 = <numeric substitution> = <total_base> <бал/бали/балів>
```

Result item:

```text
Початкова група: <1/2/3/4>; S_tot,base = <total_base> <бал/бали/балів>.
```

Note:

```text
Група 1 — S_tot > 26; група 2 — 23 ≤ S_tot ≤ 26; група 3 — 19 ≤ S_tot ≤ 22; група 4 — S_tot ≤ 18.
```

### 5. Розрахунковий опір сталі R_y
...
Характеристичний опір за класом міцності: R_yn = <value> МПа
Коефіцієнт надійності за матеріалом за відповідним рядком таблиці 7.2: γ_m = <1,025/1,050/1,100>
```

Formula:

```text
R_y = R_yn / γ_m = <Ryn> / <gamma_m> = <Ry> МПа
```

If the selected stress display unit is not MPa, append:

```text
= <R_y_display> <selected_unit>
```
...
Automatic mode, no special value applies for another structure:

```text
Для конструкції «<structure_label>» за умов «<load_description>» спеціальні значення коефіцієнта умов роботи за таблицею 5.1 не застосовуються. Відповідно до примітки 5 прийнято γ_c = 1,0.
```

Automatic mode, one special position applies:

```text
Для конструкції «<structure_label>» встановлено такі визначальні ознаки: <applicability_facts>. За позицією <position> таблиці 5.1 прийнято γ_c = <value>.
```

Automatic mode, an allowed combination applies:

```text
Для конструкції «<structure_label>» встановлено такі визначальні ознаки: <applicability_facts>. Відповідно до примітки <note_number> сумісно застосовано позиції <positions>; прийнято γ_c = <value_1> * <value_2> = <result>.
```

Semi-automatic mode:

```text
За обраною позицією <position> таблиці 5.1 прийнято γ_c = <selected_value>. Застосовність цієї позиції перевіряє користувач.
```

Manual mode:

```text
Прийнято користувачем γ_c = <manual_value>. Нормативну застосовність значення користувач перевіряє самостійно.
```

Manual-mode error:

```text
Коефіцієнт умов роботи γ_c у ручному режимі має бути більше 0.
```

Notes:

```text
Коефіцієнти γ_c < 1,0 не враховуються сумісно, крім випадків приміток 2 і 3 до таблиці 5.1.
Дозволені сполучення за приміткою 2: позиції 6 і 1; 6 і 2; 6 і 5.
Дозволені сполучення за приміткою 3: позиції 9 і 2; 9 і 3.
Якщо одночасно застосовні кілька альтернативних коефіцієнтів, сумісне врахування яких не дозволене, приймається найменше значення γ_c.
```

Warning:

```text
Для опорної плити завтовшки понад 80 мм таблиця 5.1 не встановлює значення γ_c у позиції 9; застосовність коефіцієнта потрібно обґрунтувати окремо.
```
...
σ_dyn — найбільший модуль нормального розтягувального напруження від динамічної складової навантаження.
σ_sum — найбільший модуль сумарного нормального розтягувального напруження від усіх розрахункових навантажень у тій самій точці перерізу.
α = |σ_dyn| / |σ_sum| = <sigma_dyn> / <sigma_sum> = <alpha>
```

Dynamic load result items:

```text
За α = <alpha> конструкцію віднесено до категорії <I/II/III> за напруженим станом; прийнято S_3,A2 = <8/5/1> <бал/бали/балів>.
```
...
```text
Категорія за напруженим станом не уточнюється; прийнято S_3,A2 = S_3,base = <value> <бал/бали/балів>.
```
...
```text
Для визначення α найбільше сумарне розтягувальне напруження σ_sum має бути більшим за нуль.
Напруження σ_dyn не може перевищувати σ_sum, оскільки динамічна складова входить до сумарного розтягувального напруження від усіх навантажень.
```
...
```text
ΔS_t = <0/+1/+2> <бал/бали/балів> — поправка за товщиною прокату t = <t_mm> мм.
ΔS_guillotine = <+1/0> <бал/бали/балів> — поправка за наявністю кромок після гільйотинного різання.
ΔS_cold = <+1/0> <бал/бали/балів> — поправка за неврахованим наклепом.
ΔS_initial = <+1/0> <бал/бали/балів> — поправка за високими початковими напруженнями.
```
...
Static load without tensile stress, definition:

```text
σ_c — модуль нормального напруження стиску, визначений з урахуванням коефіцієнтів φ, φ_e і φ_b.
σ_limit = 0,4 * R_y * γ_c = 0,4 * <Ry_display> * <gamma_c> = <sigma_limit_display> <selected_unit>
```

Passing comparison formula:

```text
σ_c = <sigma_c_display> <selected_unit> ≤ σ_limit = <sigma_limit_display> <selected_unit>
```

Failing comparison formula:
...
```text
Умову зменшення показника групи при статичному стиску <виконано/не виконано>; прийнято ΔS_compression = <-4/0> <бал/бали/балів>.
```

Dynamic load formula:

```text
Зменшення показника для статичного стиску не застосовується при динамічному навантаженні. Прийнято ΔS_compression = 0 балів.
```

Note:

```text
σ_c вводиться як невід’ємний модуль нормального напруження стиску, обчисленого з урахуванням коефіцієнтів φ, φ_e і φ_b.
```

Error:

```text
Нормальне напруження стиску σ_c має бути невід’ємним значенням.
```
...
```text
Уточнена група: <1/2/3/4>; S_tot,A2 = <total_a2> <бал/бали/балів>.
```

Notes:

```text
Загальне коригування показника S_tot після підбору перерізу обмежується діапазоном від −4 до +4 балів.
...
```text
Перевірено рядок таблиці Г.1 для сталі <steel_class>, групи <group>, виду прокату <product_type>, товщини <t_display> та умов експлуатації <service_condition>. Застосування сталі <steel_class> <допускається/не допускається>.
```
...
Початкова група: <base_group>; S_tot,base = <value> <бал/бали/балів>.
Уточнена група: <refined_group>; S_tot,A2 = <value> <бал/бали/балів>.
Застосування сталі <steel_class> <допускається/не допускається> за таблицею Г.1.
```
...
Початкова група: <1/2/3/4>, S_tot,base = <value>
Уточнена група: <1/2/3/4>, S_tot,A2 = <value>
Коефіцієнт умов роботи: γ_c = <value>
Розрахунковий опір сталі: R_y = <value unit>
...
Коефіцієнти умов роботи γ_c та правила їх сумісного врахування за примітками 1–5.

ДБН В.2.6-198:2014, пункти 7.1–7.2, таблиці 7.1–7.2
Визначення R_y = R_yn / γ_m і коефіцієнта надійності за матеріалом γ_m.
